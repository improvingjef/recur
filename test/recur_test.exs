defmodule RecurTest do
  ...
  doctest Recur
    ...
    test "Every other month on the 1st and last Sunday of the month for 10" do
      ...
      result = RR.unfold(%{start_date: ~D[1997-09-07], frequency: :monthly, interval: 2, count: Enum.count(expect), by_day: [{1,:sunday},{-1,:sunday}]})
      assert expect == result |> Enum.take(Enum.count(expect))
    end
    ...
    test "The 3rd instance into the month of one of Tuesday, Wednesday or Thursday, for the next 3 months" do
      ...
      results =
        RR.unfold(%{start_date: ~D[1997-09-04], frequency: :monthly, count: 3, by_day: [:tuesday, :thursday, :wednesday], by_set_position: [3]})
        |> Enum.to_list()
      assert expected == results
    end
    ...
    test "The 2nd to last weekday of the month" do
      ...
      results = RR.unfold(%{start_date: ~D[1997-09-29], frequency: :monthly, by_day: [:monday, :tuesday, :wednesday, :thursday, :friday], by_set_position: [-2]})
      |> Enum.take(7)
      assert expected == results
    end
    ...
    test "An example where the days generated makes a difference because of WKST" do
      rules = %{start_date: ~D[1997-08-05], frequency: :weekly, interval: 2, count: 4, by_day: [:tuesday, :sunday], week_start: :monday}

      monday_result = RR.unfold(rules)

      monday_expect = date_expand([{1997, 8, Enum.to_list([5,10,19,24])},])

      sunday_result = RR.unfold(%{rules | week_start: :sunday})
